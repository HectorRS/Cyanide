...
namespace Cyanide
{
    public class CyanModuleBase : ModuleBase<CyanCommandContext>
    {
        ...
        //DM replies
        public async Task<IUserMessage> ReplyDMAsync(Embed embed = null, RequestOptions options = null)
        {
            return await Context.User.SendMessageAsync("", false, embed, options).ConfigureAwait(false);
        }

        public async Task<IUserMessage> ReplyDMAsync(string message, Embed embed = null, RequestOptions options = null)
        {
